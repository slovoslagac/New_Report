...
and sc.source_id = un.source
and un.dom_id = st.team_id
and un.source = st.source_id
and un.gost_id = st1.team_id
and un.source = st1.source_id
on DUPLICATE KEY update match_id=match_id;
insert into src_subgames (game, subgame, source_id)
select distinct game, subgame, source 
from ulaz_odds
where (game, subgame, source) not in (select game, subgame, source from src_subgames);
